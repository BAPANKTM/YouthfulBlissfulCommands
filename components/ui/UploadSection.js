...
export default function UploadSection() {
  ...
  const handleUploadClick = () => {
    ...
    if (uploadRef.current) {
      uploadRef.current.style.animation = 'none';
      setTimeout(() => {
        uploadRef.current.style.animation = 'pulse 2s infinite ease-in-out, float 3s infinite ease-in-out';
      }, 10);
    }
  };

  return (
    <div className={styles.uploadSection} ref={uploadRef} onClick={handleUploadClick}>
